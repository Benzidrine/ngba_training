...
Qwen3.6-35B-A3B fine-tuning. All weights updated. No chain rule between layers.
4-bit quantized. 19GB GPU. ~40s/step. Single RTX 4090.
"""
import torch,torch.nn.functional as F,time,os
from transformers import AutoModelForCausalLM,AutoTokenizer,BitsAndBytesConfig

DEVICE='cuda';SEQ=64;BS=1;N_STEPS=5;GPU_LAYERS=10
...
for li in range(GPU_LAYERS):
    m.model.layers[li].to(DEVICE)

tok=AutoTokenizer.from_pretrained('Qwen/Qwen3.6-35B-A3B',trust_remote_code=True)
print(f"GPU: {torch.cuda.memory_allocated()/1e9:.1f}GB | {GPU_LAYERS} fixed + 30 paged | NBGA")

def mv(l,g):
    for p in l.parameters():p.data=p.data.to(g)
...
for step in range(N_STEPS):
    ...
    with torch.no_grad():
        ...
        acts=[h.cpu()]
        for li in range(40):
            if li>=GPU_LAYERS:mv(m.model.layers[li],DEVICE)
            o=m.model.layers[li](h,pe);h=o[0]if isinstance(o,tuple)else o
            acts.append(h.cpu())
            if li>=GPU_LAYERS:mv(m.model.layers[li],'cpu');torch.cuda.empty_cache()
        hf=m.model.norm(h)
...
print(f"Done. {time.time()-st:.0f}s total",flush=True)
